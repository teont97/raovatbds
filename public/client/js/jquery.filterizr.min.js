(function(global,$){'use strict';if(!$)throw new Error('Filterizr requires jQuery to work.');var Packer=function(w){this.init(w)};Packer.prototype={init:function(w){this.root={x:0,y:0,w:w}},fit:function(blocks){var n,node,block,len=blocks.length;var h=len>0?blocks[0].h:0;this.root.h=h;for(n=0;n<len;n++){block=blocks[n];if((node=this.findNode(this.root,block.w,block.h)))
  ...
  else{self._handleFiltering(array)}}},shuffle:function(){var self=this;self._isAnimating=!0;self._isShuffling=!0;self.trigger('shufflingStart');self._mainArray=self._fisherYatesShuffle(self._mainArray);self._subArrays=self._makeSubarrays();var target=self._multifilterModeOn()?self._makeMultifilterArray():self._getCollectionByFilter(self.options.filter);if(self._isSearchActivated())
  ...
  return ret},_onTransitionEnd:function(){var self=this;if(self._filteringOut){$(self).addClass('filteredOut');self._isFilteredOut=!0;self._filteringOut=!1}
  else if(self._filteringIn){self._isFilteredOut=!1;self._filteringIn=!1}
